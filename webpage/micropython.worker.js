...
class LLVM {
    ...
    async init() {
        ...
        let out = await llvm.run("make","V=1","PYTHON=python","--dry-run");
        console.log(out);

        //this.fileSystem.mkdir('/src/codal_port/build/genhdr/qstr');
        // out = await llvm.run('python','../../lib/micropython/py/makeqstrdefs.py','split','qstr','build/genhdr/qstr.i.last','build/genhdr/qstr','_');
        // console.log(out);

        // out = await llvm.run('python','../../lib/micropython/py/makeqstrdefs.py','cat','qstr','_','build/genhdr/qstr','build/genhdr/qstrdefs.collected.h');
        // console.log(out);

        // let x = new TextDecoder();
        // console.log(x.decode(this.fileSystem.readFile('/src/codal_port/build/genhdr/qstr.i.last')));
    ...
    async run(...args) {
        let process = null;
        if(args[0] === "python") process = "python";
        else if(args[0] === "make") process = "make";
        else process = "llvm-box";

        return await this.tools[process].exec(args, {
